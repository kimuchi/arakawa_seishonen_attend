...
async function callApi(fnName, ...args) {
  ...
  switch (fnName) {
    ...
    case 'api_getEventStats':
      resp = await httpJson('GET', '/api/events/stats'); break;
    case 'api_addEvent':
...
async function renderEventsTab() {
  ...
  panel.innerHTML = `
    <div class="card">
      ...
      <div id="evTotals" class="ev-totals"></div>
  ...
  renderEventRows_();
  // 出席/欠席人数・日当は別途取得して再描画 (一覧表示を待たせない)
  await reloadEventStats_();
}

/**
 * イベント別の出席/欠席人数と日当単価を取得し、一覧を再描画する。
 */
async function reloadEventStats_() {
  try {
    APP._eventStats = await callApi('api_getEventStats');
  } catch (e) {
    APP._eventStats = null;
  }
  if (document.getElementById('eventsList')) renderEventRows_();
}

/**
 * 指定イベントの出席/欠席人数と日当額を返す。
 * 日当額は「日当対象」イベントのみ 出席人数 × 日当単価 で算出する。
 */
function eventStatOf_(ev) {
  const loaded = !!APP._eventStats;
  const src = (APP._eventStats && APP._eventStats.stats) || {};
  const s = src[ev.id] || { attended: 0, absent: 0 };
  const rate = (APP._eventStats && Number(APP._eventStats.dailyAllowance))
    || Number(APP.settings['日当単価']) || 500;
  ...
  return {
    ...
    absent,
    rate,
    amount: ev.dailyAllowance ? attended * rate : 0,
  };
}
...
function renderEventRows_() {
  ...
  // 絞り込み結果の合計 (出席/欠席の延べ人数・日当合計)
  const totals = list.reduce((a, ev) => {
    const st = eventStatOf_(ev);
    a.attended += st.attended;
    a.absent += st.absent;
    a.amount += st.amount;
    return a;
  }, { attended: 0, absent: 0, amount: 0 });
  const statsLoaded = !!APP._eventStats;
  const totalsBox = document.getElementById('evTotals');
  if (totalsBox) {
    totalsBox.innerHTML = statsLoaded ? `
      <div class="ev-totals__item">
        <span class="ev-totals__label">出席 (延べ)</span>
        <span class="ev-totals__num ev-totals__num--ok">${totals.attended}<small>名</small></span>
      </div>
      <div class="ev-totals__item">
        <span class="ev-totals__label">欠席 (延べ)</span>
        <span class="ev-totals__num ev-totals__num--ng">${totals.absent}<small>名</small></span>
      </div>
      <div class="ev-totals__item ev-totals__item--money">
        <span class="ev-totals__label">日当合計</span>
        <span class="ev-totals__num">¥${totals.amount.toLocaleString()}</span>
      </div>
    ` : '<div class="ev-totals__loading">集計を読み込み中...</div>';
  }
  ...
  box.innerHTML = list.map(ev => {
    const st = eventStatOf_(ev);
    const statsHtml = st.loaded ? `
      <div class="event-item__stats">
        <div class="event-stat">
          <span class="event-stat__num event-stat__num--ok">${st.attended}</span>
          <span class="event-stat__label">出席</span>
        </div>
        <div class="event-stat">
          <span class="event-stat__num event-stat__num--ng">${st.absent}</span>
          <span class="event-stat__label">欠席</span>
        </div>
        <div class="event-stat event-stat--money">
          <span class="event-stat__num">${ev.dailyAllowance ? '¥' + st.amount.toLocaleString() : '—'}</span>
          <span class="event-stat__label">日当</span>
        </div>
      </div>
    ` : '<div class="event-item__stats event-item__stats--loading">…</div>';
    return `
    <div class="event-item">
      <div class="event-item__date">${formatDateBadge_(ev.date)}</div>
      <div class="event-item__body">
